...
const DATA_PATH = path.resolve(process.cwd(), "data", "customers-from-excel-final.json");

async function main() {
  console.log("👥 Importando clientes finais (filtrados)...\n");

  if (!fs.existsSync(DATA_PATH)) {
    console.error(`❌ Arquivo não encontrado: ${DATA_PATH}`);
    console.log("   Execute primeiro: npm run extract:customers:final");
    return;
  }
