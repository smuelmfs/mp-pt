...
async function main() {
  console.log('\n📄 VALIDAÇÃO: Materiais de ALVEOLAR (Sistema vs Excel)\n');
  ...
  if (!worksheet) {
    console.error('❌ Aba ALVEOLAR não encontrada no Excel');
  ...
  if (headerRow === -1) {
    console.error('❌ Header não encontrado na aba ALVEOLAR');
  ...
  console.log(`\n📊 Excel: ${excelMaterials.length} materiais alveolares encontrados\n`);

  // Busca materiais no sistema (alveolar ou rigido)
